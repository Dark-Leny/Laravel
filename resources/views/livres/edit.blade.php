@extends('layouts.app', [
    'title' => 'Modifier le livre: ' . $livre->titre,
    'breadcrumbs' => [
        ['label' => 'Catalogue', 'url' => route('livres.index')],
        ['label' => $livre->titre, 'url' => route('livres.show', $livre->id)],
        ...
        <div class="col-12 col-lg-8 mx-auto">
            <div class="card shadow">
                <div class="card-header bg-warning text-dark">
                    <h2 class="mb-0"><i class="fas fa-edit"></i> Modifier le livre</h2>
                </div>
                <div class="card-body">
                    <form action="{{ route('livres.update', $livre->id) }}" method="POST" class="needs-validation">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('titre') is-invalid @enderror" 
                                   id="titre" name="titre" value="{{ old('titre', $livre->titre) }}" 
                            ...
                            <select class="form-select @error('categorie_id') is-invalid @enderror" 
                                ...
                                @foreach($categories as $categorie)
                                    <option value="{{ $categorie->id }}" 
                                        @if(old('categorie_id', $livre->categorie_id) == $categorie->id) selected @endif>
                                        {{ $categorie->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('annee') is-invalid @enderror" 
                                       id="annee" name="annee" value="{{ old('annee', $livre->annee) }}" 
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('nb_pages') is-invalid @enderror" 
                                       id="nb_pages" name="nb_pages" value="{{ old('nb_pages', $livre->nb_pages) }}" 
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('resume') is-invalid @enderror" 
                                      ...
                                      placeholder="Entrez le résumé du livre...">{{ old('resume', $livre->resume) }}</textarea>
                            @error('resume')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="disponible" 
                                       ...
                                       @if(old('disponible', $livre->disponible)) checked @endif>
                        ...
                        <div class="d-grid gap-2 d-sm-flex justify-content-sm-end mt-4">
                            <a href="{{ route('livres.show', $livre->id) }}" class="btn btn-secondary">
                                <i class="fas fa-times"></i> Annuler
                            </a>
                            <button type="submit" class="btn btn-warning">
                                <i class="fas fa-save"></i> Modifier le livre
                            </button>
                        </div>
                    </form>
                </div>
